            ...
            if getattr(st.session_state["last_result"], "market_used", None):
                with st.expander("Market context used (debug)"):
                    ...
                    with c2:
                        ...
                        st.metric("Last price", f"{float(lp):.2f}" if lp is not None else "n/a")
                    with c3:
                        v = mu.get("vol_per_sqrt_day_est")
                        st.metric("Vol (est)", f"{float(v):.4f}" if v is not None else "n/a")
...
with tab_market:
    ...
    st.caption("This sim is for demo only. It does not represent real markets.")
    ...
    frame = ticks_to_frame(ticks) if ticks else pd.DataFrame(
        columns=["ts", "ticker", "price", "volume"])

    with col_b:
        ticker = st.selectbox(
            "Ticker", options=[t.ticker for t in default_universe()], index=0)

        if not frame.empty:
            latest = frame[frame["ticker"] == ticker].tail(1)
            if not latest.empty:
                st.metric("Last price",
                          f"{float(latest.iloc[0]['price']):.2f}")
        else:
            st.info("Click 'Tick once' to generate prices.")

    with col_c:
        if not frame.empty:
            sub = frame[frame["ticker"] == ticker].tail(240)
            if not sub.empty:
                fig = px.line(sub, x="ts", y="price",
                              title=f"{ticker} price (simulated)")
                fig.update_layout(margin=dict(
                    l=10, r=10, t=40, b=10), height=340)
    ...
    if not frame.empty:
        ...
        if events.empty:
            st.write("No events yet for this ticker.")
        else:
            st.dataframe(
                events[["ts", "event_type", "headline", "impact"]],
